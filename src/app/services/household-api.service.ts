...
export class HouseholdApiService {
  ...
  getCurrent(): Observable<HouseholdRelations> {
    return this.http.get<HouseholdRelations>(`${this.apiUrl}/household`, {
      headers: this.authHeaders(),
    });
  }

  update(data: Partial<Household>): Observable<Household> {
    return this.http.patch<Household>(`${this.apiUrl}/household`, data, {
      headers: this.authHeaders(),
    });
  }

  delete(): Observable<{ success: boolean } | Household> {
    return this.http.delete<{ success: boolean } | Household>(`${this.apiUrl}/household`, {
      headers: this.authHeaders(),
    });
  }
}
