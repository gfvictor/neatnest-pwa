...
export class ObjectApiService {
  ...
  listByContainer(containerId: string): Observable<Obj[]> {
    return this.http.get<Obj[]>(`${this.apiUrl}/object/container/${containerId}`, {
      headers: this.authHeaders(),
    });
  }

  getById(id: string): Observable<Obj> {
    return this.http.get<Obj>(`${this.apiUrl}/object/${id}`, { headers: this.authHeaders() });
  }
  ...
  delete(id: string): Observable<{ success: boolean } | Obj> {
    return this.http.delete<Obj>(`${this.apiUrl}/object/${id}`, { headers: this.authHeaders() });
  }
}
